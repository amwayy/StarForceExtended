...
namespace StarForce
{
    ...
    public abstract class Aircraft : TargetableObject
    {
        ...
        public void AttachWeapon(int typeId) {
            WeaponData weaponData = new WeaponData(GameEntry.Entity.GenerateSerialId(), typeId, Id, m_AircraftData.Camp);
            m_AircraftData.AttachWeaponData(weaponData);
            GameEntry.Entity.ShowWeapon(weaponData);
        }

        public float GetWeaponNextAttackTime() {
            if (m_Weapons == null || m_Weapons.Count == 0) return 0f;

            return m_Weapons[0].GetNextAttackTime();
        }

        public int GetWeaponIndex(Weapon weapon) {
            return m_Weapons.IndexOf(weapon);
        }
